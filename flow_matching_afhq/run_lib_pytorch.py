...
def train(config, workdir):
    ...
    # If downsampling is enabled, update config.data.image_size for model creation
    effective_image_size = config.data.get('downsample_size', None)
    if effective_image_size is not None:
        original_image_size = config.data.image_size
        config.data.image_size = effective_image_size
        logging.info(f"Model will be created for {effective_image_size}x{effective_image_size} (downsampled from {original_image_size}x{original_image_size})")
    ...
    if config.training.snapshot_sampling:
        # Use downsampled size if specified, otherwise use original image_size
        effective_image_size = config.data.get('downsample_size', config.data.image_size)
        if effective_image_size is None:
            effective_image_size = config.data.image_size
    ...
    for ckpt in range(begin_ckpt, config.eval.end_ckpt + 1):
        ...
        if config.eval.enable_sampling:
            ...
            try:
                ...
                logging.warning("tensorflow_gan not available, skipping FID/KID/IS computation")
                inception_score = -1
                fid = -1
                kid = -1
